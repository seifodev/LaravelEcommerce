...
class UserDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables($query)
            ->addColumn('action', 'admin.users.action.edit')
            ->editColumn('updated_at', function ($admin) {
                return $admin->updated_at ?  $admin->updated_at->diffForHumans() : NULL;
            })
            ->editColumn('created_at', function ($admin) {
                return $admin->created_at ?  $admin->created_at->diffForHumans() : NULL;
            })
            ->editColumn('level', 'admin.users.action.level')
    ...
    {
        return $model->orderBy('id', 'desc')->where(function ($builder) {
            return request()->has('level') ? $builder->where('level', request('level')) : $builder;
        });
    }
    ...
    protected function getColumns()
    {
        return [
            ...
            [
                'data' => 'name',
                'name' => 'name',
                'title' => trans('admin.table.name')
            ],
            [
                'data' => 'email',
                'name' => 'email',
                'title' => trans('admin.table.email')
            ],
            [
                'data' => 'level',
                'name' => 'level',
                'title' => trans('admin.userLevel.level')
            ],
            [
                'data' => 'created_at',
                'name' => 'created_at',
    ...
    protected static function btns()
    {
        return [
            'dom' => [
                ...
                'buttons' => [
                    ['text' => '<i class="fa fa-plus"></i>', 'className' => 'btn btn-primary', 'action' => 'function (){window.location.href = "' . route('users.create') . '"}'],
