...
class ShippingDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables($query)
            ->addColumn('action', 'admin.shippings.action.edit')
            ->editColumn('updated_at', function ($shipping) {
                return $shipping->updated_at ?  $shipping->updated_at->diffForHumans() : NULL;
            })
            ->editColumn('created_at', function ($shipping) {
                return $shipping->created_at ?  $shipping->created_at->diffForHumans() : NULL;
            })
            ->editColumn('user_id', function ($shipping) {

            })
            ->editColumn('logo', 'admin.shippings.action.logo')
            ->addColumn('checkbox', 'admin.shippings.action.checkbox')
    ...
    {
        return $model->orderBy('id', 'desc')->with('user');
    }
    ...
    protected static function btns()
    {
        return [
            'dom' => [
                ...
                'buttons' => [
                    ['text' => '<i class="fa fa-plus"></i>', 'className' => 'btn btn-primary', 'action' => 'function (){window.location.href = "' . route('shippings.create') . '"}'],
