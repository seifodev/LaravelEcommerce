...
class SizeDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables($query)
            ->addColumn('action', 'admin.sizes.action.edit')
            ->editColumn('updated_at', function ($size) {
                return $size->updated_at ?  $size->updated_at->diffForHumans() : NULL;
            })
            ->editColumn('created_at', function ($size) {
                return $size->created_at ?  $size->created_at->diffForHumans() : NULL;
            })
    ...
    {
        return $model->orderBy('id', 'desc')->with('department');
    }
    ...
    protected function getColumns()
    {
        return [
            ...
            [
                'data' => 'department',
                'name' => 'department',
                'title' => trans('admin.form.department')
            ],
            [
                'data' => 'created_at',
                'name' => 'created_at',
    ...
    protected static function btns()
    {
        return [
            'dom' => [
                ...
                'buttons' => [
                    ['text' => '<i class="fa fa-plus"></i>', 'className' => 'btn btn-primary', 'action' => 'function (){window.location.href = "' . route('sizes.create') . '"}'],
