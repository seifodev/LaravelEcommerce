...
class WeightDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables($query)
            ->addColumn('action', 'admin.weights.action.edit')
            ->editColumn('updated_at', function ($weight) {
                return $weight->updated_at ?  $weight->updated_at->diffForHumans() : NULL;
            })
            ->editColumn('created_at', function ($weight) {
                return $weight->created_at ?  $weight->created_at->diffForHumans() : NULL;
            })
            ->addColumn('checkbox', 'admin.weights.action.checkbox')
    ...
    protected static function btns()
    {
        return [
            'dom' => [
                ...
                'buttons' => [
                    ['text' => '<i class="fa fa-plus"></i>', 'className' => 'btn btn-primary', 'action' => 'function (){window.location.href = "' . route('weights.create') . '"}'],
