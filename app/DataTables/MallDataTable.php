...
class MallDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables($query)
            ->addColumn('action', 'admin.malls.action.edit')
            ->editColumn('updated_at', function ($mall) {
                return $mall->updated_at ?  $mall->updated_at->diffForHumans() : NULL;
            })
            ->editColumn('created_at', function ($mall) {
                return $mall->created_at ?  $mall->created_at->diffForHumans() : NULL;
            })
            ->addColumn('country', function ($mall) {
                return $mall->country->{'name_' . lang()};
            })
            ->editColumn('logo', 'admin.malls.action.logo')
    ...
    {
        return $model->orderBy('id', 'desc')->with('country');
    }
    ...
    protected function getColumns()
    {
        return [
            ...
            [
                'data' => 'country',
                'name' => 'country',
                'title' => trans('admin.form.country')
            ],
            [
                'data' => 'logo',
                'name' => 'logo',
    ...
    protected static function btns()
    {
        return [
            'dom' => [
                ...
                'buttons' => [
                    ['text' => '<i class="fa fa-plus"></i>', 'className' => 'btn btn-primary', 'action' => 'function (){window.location.href = "' . route('malls.create') . '"}'],
