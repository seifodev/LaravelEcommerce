...
class ColorDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables($query)
            ->addColumn('action', 'admin.colors.action.edit')
            ->editColumn('updated_at', function ($color) {
                return $color->updated_at ?  $color->updated_at->diffForHumans() : NULL;
            })
            ->editColumn('created_at', function ($color) {
                return $color->created_at ?  $color->created_at->diffForHumans() : NULL;
            })
            ->editColumn('color', 'admin.colors.action.color')
            ->addColumn('checkbox', 'admin.colors.action.checkbox')
            ->rawColumns(['action', 'checkbox', 'color'])
            ;
    }
    ...
    protected static function btns()
    {
        return [
            'dom' => [
                ...
                'buttons' => [
                    ['text' => '<i class="fa fa-plus"></i>', 'className' => 'btn btn-primary', 'action' => 'function (){window.location.href = "' . route('colors.create') . '"}'],
